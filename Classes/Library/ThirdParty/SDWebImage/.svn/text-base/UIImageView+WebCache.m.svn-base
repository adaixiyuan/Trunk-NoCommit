...
- (void)setImageWithURL:(NSURL *)url options:(SDWebImageOptions)options progress:(BOOL)progress{
    if(progress){
        ...
        [self setImageWithURL:url success:^(UIImage *image, BOOL cached) {
            ...
            self.image = [UIImage noCacheImageNamed:@"placeHolder_bg.png"];
            self.contentMode = UIViewContentModeCenter;
        }];
	}else{
        [self setImageWithURL:url placeholderImage:[UIImage noCacheImageNamed:@"bg_nohotelpic.png"] options:options];
    }
}
