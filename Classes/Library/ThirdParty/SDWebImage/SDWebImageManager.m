...
//            userInfo = nil;
//        }
        NSDictionary *userInfo = info;//[info objectForKey:@"userInfo"];
...
- (void) imageDownloader:(SDWebImageDownloader *)downloader didChangedProgress:(float)progress{
    // Notify all the downloadDelegates with this downloader
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        NSUInteger uidx = (NSUInteger)idx;
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didChangedProgress:)])
            {
                //objc_msgSend(delegate, @selector(webImageManager:didChangedProgress:), self,progress);
                [delegate webImageManager:self didChangedProgress:progress];
            }
        }
    }
}
