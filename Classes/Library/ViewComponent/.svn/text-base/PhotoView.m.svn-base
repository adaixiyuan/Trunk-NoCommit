...
@synthesize scaleable;
@synthesize progress;


- (void) dealloc{
    for (UIView *photoView in photosView.subviews) {
        if ([photoView isKindOfClass:[UIScrollView class]]) {
            ((UIScrollView *)photoView).delegate = nil;
        }
    }
...
- (id)initWithFrame:(CGRect)frame photoUrls:(NSArray *)urs progress:(BOOL) progress_{
    ...
    if (self) {
        ...
        for (int i = 0; i < PHOTOVIEW_CACHECOUNT; i++) {
            ...
            // 双击手势
            doubleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(imgDoubleTap:)];
            ...
            // 单击手势
            singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(imgSingleTap:)];
            ...
            [imageView setImageWithURL:[NSURL URLWithString:[self.photoUrls safeObjectAtIndex:i]]
...
- (void) setImageContentMode:(UIViewContentMode)imageContentMode{
    ...
    if (photoArray) {
        for (int i = 0; i < photoArray.count; i++) {
            UIImageView *imageView = (UIImageView *)[photoArray safeObjectAtIndex:i];
            imageView.contentMode = self.imageContentMode;
        }
    }
}
...
- (void) photoPageUp{
    ...
    UIImageView *imageView =[photoArray safeObjectAtIndex:0];
    UIScrollView *scrollView = (UIScrollView *)[imageView superview];

    [imageView setImageWithURL:[NSURL URLWithString:[self.photoUrls safeObjectAtIndex:index]]
                       options:SDWebImageCacheMemoryOnly
                      progress:self.progress];
...
- (void) photoPageDownSilently{
    ...
    UIImageView *imageView =[photoArray safeObjectAtIndex:PHOTOVIEW_CACHECOUNT - 1];
    UIScrollView *scrollView = (UIScrollView *)[imageView superview];
...
- (void) pageToIndex:(NSInteger)_index{
    if (_index == 0) {
        photosView.contentOffset = CGPointZero;
    }else if(_index == 1){
...
-(void) scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ([delegate respondsToSelector:@selector(photoView:didScrollOffsetX:contentWidth:)]) {
        [delegate photoView:self didScrollOffsetX:scrollView.contentOffset.x contentWidth:scrollView.contentSize.width];
    }
}
...
- (void) layoutSubviews{
    if (photoSize.height == self.frame.size.height) {
        return;
    }
    ...
    for (int i = 0; i < PHOTOVIEW_CACHECOUNT; i++) {
        UIScrollView *scaleView = (UIScrollView *)[((UIImageView *)[photoArray safeObjectAtIndex:i]) superview];
