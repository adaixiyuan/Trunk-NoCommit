...
@implementation CashAccountTable
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row)
    {
        ...
        {
            // 显示订单总价
            if (_fromType == UniformFromTypeFlight)
            {
                // 机票流程
            ...
            else
            {
                ...
                cell = (CashAccountTableCell *)[tableView dequeueReusableCellWithIdentifier:identifier0];
                ...
                if (self.fromType==UniformFromTypeHotel)
                {
                    ...
                    float realPrice = exchangeRate>0?_totalPrice*exchangeRate:_totalPrice;
                    ...
                    if ([RoomType isPrepay]) {
                        ...
                        if (coupons.count) {
                            NSInteger coupon = [[[coupons objectAtIndex:0] objectForKey:@"CouponValue"] intValue];
                            realPrice = realPrice - coupon;
                        }
                    }
                    
                    //更改bug，担保都是人民币
                    cell.priceLabel.text = [NSString stringWithFormat:@"%@%.2f",CURRENCY_RMBMARK,realPrice];
                }
                else
                {
                    cell.priceLabel.text = [NSString stringWithFormat:@"¥ %.2f", _totalPrice];
